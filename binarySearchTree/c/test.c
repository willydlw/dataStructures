...
void testInsertion(int *arr, int n)
{
    {   // test iterative insert
        struct TreeNode *root = createTree(bstInsertIterative, arr, n);
        for(int i = 0; i < n; ++i)
        {
            struct TreeNode* result = bstSearchIterative(root, arr[i]);
    ...
    {   // test recursive insert and search
        struct TreeNode *root = createTree(bstInsertRecursive, arr, n);
        for(int i = 0; i < n; ++i)
        {
            struct TreeNode* result = bstSearchRecursive(root, arr[i]);
            assert(result->val == arr[i]);
        }
        deletePostOrder(root);
        assert(root == NULL);
...
void testSearch()
{
    {   // search empty tree using iterative search method
        ...
        result = bstSearchIterative(root, 1);
        assert(result == NULL);
    }
    ...
    {   // search empty tree using recursive search method
        struct TreeNode* root = NULL;
        struct TreeNode* result;
        result = bstSearchRecursive(root, 1);
        assert(result == NULL);
    }
    ...
    {   // search for value not stored in tree
        ...
        {   // iterative functions
            struct TreeNode *root = NULL;
            root = createTree(bstInsertIterative, a, n);
            ...
            struct TreeNode *result;  
            result = bstSearchIterative(root, valueNotInArray);
            assert(result == NULL);
        ...
        {   // recursive functions
            struct TreeNode *root = NULL;
            root = createTree(bstInsertRecursive, a, n);

            // tree should only contain positive numbers, so
            // searching for a negative value should produce a NULL result
            int valueNotInArray = -(rand() + 1);  
            struct TreeNode *result;  
            result = bstSearchRecursive(root, valueNotInArray);
            assert(result == NULL);
            root = deletePostOrder(root);
            assert(root == NULL);
        }
...
void testDelete(void)
{
    ...
    {   // test deletion left leaf node
        ...
        root = createTree(bstInsertRecursive, arr, 3);

        // find value in tree
        struct TreeNode* found = bstSearchRecursive(root, valueToDelete);
...
void multiTest(int n, int *arr, int *inorder, int *preorder, int *postorder)
{
    {   // iterative insertion and search
        struct TreeNode *root = NULL;
        root = createTree(bstInsertIterative, arr, n);
        assert(root != NULL);

        for(int i = 0; i < n; ++i)
        {
            struct TreeNode *result = NULL;
            result = bstSearchIterative(root, arr[i]);
            assert(result->val == arr[i]);
        }
